...
int main()
{
    ...
    //CHEQUEAR QUE DEPSUES DE LLENAR EL AVION SALGA CARTEL AVION LLENO
    int avion[3][10];
    ...
    recaudacion1=0;
    recaudacion2=0;
    totaleconomico=0; //Chequear la suma total de recaudacion da 0
    totalprimera=0;
    totalturista=0;
    b =0;
    a=0;
    ...
    case 1:
            ...
            printf("POR UN MONTO DE $ 8000 \n");
                  for (i=0;i<1;i++){ //RECORRIDO DE PRIMERA CLASE
                for(j=0;j<10;j++){
                    if(avion[i][j]==1){
            ...
            for(i=0; i<1; i++){
                for (j=0; j<10; j++){ // Chequear que cuadno compro todos los asientos de una fila esta condicion no me vende
                  if ((avion[i][j]==0) &&(b==0)){   // las demas filas*/
                printf("\nSE LE HA ASIGNADO EL ASIENTO: %d",j+1 ); // chequear llenado de asientos
                    avion[i][j]=1;
                    b=1;
    ...
    case 2:
        ...
        printf("USTED HA SELLECIONADO CLASE TURISTA \n");
           ...
           for (i=1;i<2;i++){ //RECORRIDO DE CLASE TURISTA
                ...
                if ((acumuladorde2==10)&&(acumuladorde3<10)){ //chequeo escases de asientos primera clase%25
          ...
          for(i=1; i<2; i++){
                ...
                printf("\nSE LE HA ASIGNADO EL ASIENTO: %d",j+1 );
                    ...
                    pasaje2=turista;//acumulador de precio
                    totalpasaje2=pasaje2+totalpasaje2; //total del precio
                                      }
                }
            }
    ...
    case 5:
        b=0;
        for(i=0;i<1;i++){ // RECAUDACION PRIMERA CLASE
            for(j=0;j<10;j++){
                if (avion[i][j]==1){
    ...
    case 6:
        ...
        case 1:
         b=0;
            //DECOLUCION PRIMERA CLASE CONTROLADORES
                for (j=9; j>=0; j--){
                      if ((avion[0][j]==1) &&(b==0)){
                printf("\nSE LE DEVOLVERA EL VALOR DEL PASAJE POR:$ %f",primera);
                    avion[0][j]=0;
                    b=1;
        ...
        case 2:
                    b=0;
                     //DEVOLUCION TURISTA CONTROLADORES  //CONTROLAR TURISTA Y PRIMERA CLASE DEVOLUCION
                        for (j=9; j>=0; j--){
                            if ((avion[1][j]==1) &&(b==0)){
                                printf("\nSE LE DEVOLVERA EL VALOR DEL PASAJE POR:$ %f", turista);
                                    avion[1][j]=0;
                                b=1;
                            totalpasaje2 = totalpasaje2 - turista;
                                comprapasajeT = comprapasajeT-1;
                    }
                    }
        ...
        case 3:
            b=0;
             //DEVOLUCION ECONOMICO CONTROLADORES
                for (j=9; j>=0; j--){
                      if ((avion[2][j]==1) &&(b==0)){ //las demas filas*/
                printf("\nSE LE DEOLVERA EL VALOR DEL PASAJE POR:$ %f",economico ); // chequear llenado de asientos
                    avion[3][j]=0;
                    b=1;
    ...
    case 7:
        ...
        break;
        default:
            printf("El valor no corresponde");
            scanf("%d",&a); }
        system("\n pause");
        system("\ncls");
    }
